...
class Optimizer(ABC):
    ...
    # whether this optimizer can learn the nugget when the model is built with
    # ``Dace(noise='auto')``. Most optimizers take a fixed numeric noise; only those that
    # search the (theta, noise) space jointly set this True (see ``ScreenedLBFGS``).
    supports_auto_noise = False
...
def fit_feasible(dace, theta, relocate=True, noise=None):
    ...
    **Noise climbing** (only if no theta in the bounds is feasible): fall back to a fit at
    ...
    Args:
        ...
        noise: Numeric nugget to fit at, overriding ``dace.noise``. Passed by optimizers that
            learn the nugget (``noise='auto'``) so the committed fit uses the *learned* value
            rather than the string ``'auto'``. ``None`` uses ``dace.noise`` (the fixed case).
    ...
    up = dace.tu
    nz = dace.noise if noise is None else noise
    ...
    for _ in range(64):
        try:
            return t_try, fit(X, Y, dace.regr, dace.kernel, t_try, noise=nz)
        except DaceFitError:
            if not relocate:
                break
    ...
    t_fb = t_try
    try:
        return t_fb, fit(X, Y, dace.regr, dace.kernel, t_fb, noise=nz, max_noise=dace.max_noise)
