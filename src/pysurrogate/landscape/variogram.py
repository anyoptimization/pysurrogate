...
def compute(ctx) -> dict:
    ...
    The empirical variogram is binned only up to the *median* pairwise distance (see
    :meth:`Context.variogram`), so all range/span-relative features are relative to that
    truncated near-field span, not the full distance range of the cloud.
    ...
    # --- correlation range: distance to reach 95% of sill, relative to the binned span (the
    #     variogram is truncated at the median pairwise distance, so 1.0 means "no decorrelation
    #     within the near field", not within the whole cloud)
    if np.isfinite(sill) and sill > 0 and h.size >= 1:
        ...
        if hit.size > 0 and np.isfinite(span) and span > 0:
            ...
            # never reaches the sill within the binned (median-distance) span -> long-range
            # correlation
            out["range_rel"] = 1.0
    ...
    #     <0 concave saturation). Normalized to [-1, 1] by the local semivariance scale. Empty bins
    #     are dropped, so the first three centers can be unevenly spaced; a plain second difference
    #     is only a meaningful curvature probe on a near-uniform grid, else it stays nan.
    if h.size >= 3:
        g3 = gamma[:3]
        d1 = float(h[1] - h[0])
        d2 = float(h[2] - h[1])
        uniform = d1 > 0 and d2 > 0 and abs(d1 - d2) <= 0.05 * max(d1, d2)
        if uniform and np.all(np.isfinite(g3)):
            second = g3[0] - 2.0 * g3[1] + g3[2]
            scale = float(np.mean(np.abs(g3))) + 1e-12
            out["near_origin_convexity"] = float(np.clip(second / scale, -1.0, 1.0))

    # --- noise slope ratio: near-origin secant slope vs the slope over the whole binned span. A
    #     large intercept (noise) flattens the near-origin rise; ratio ~1 means smooth structure.
