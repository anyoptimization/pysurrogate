#!/usr/bin/env python3
"""pysurrogate documentation builder CLI.

A standalone, isolated documentation pipeline driven by ``uvx --from ./docs
pysurrogate-docs``. pyclawd's ``pyclawd docs`` group delegates to this CLI, so all
...
log = logging.getLogger("pysurrogate-docs")
...
# so long-but-legitimate cells don't time out. Override via PYSURROGATE_DOCS_TIMEOUT.
EXEC_TIMEOUT = os.environ.get("PYSURROGATE_DOCS_TIMEOUT", "120")

# Number of notebooks executed in parallel. Default = cores - 2 (min 1); override
# via PYSURROGATE_DOCS_JOBS to keep small machines responsive.
EXEC_JOBS = os.environ.get("PYSURROGATE_DOCS_JOBS") or str(max(1, (os.cpu_count() or 3) - 2))
...
def install_project():
    ...
    try:
        import pysurrogate  # noqa: F401

        print("✅ pysurrogate is available")
        return True
    except ImportError:
        print("📦 Installing pysurrogate in development mode...")
        parent_dir = Path("..").resolve()
        if (parent_dir / "pyproject.toml").exists():
            run_command(["python", "-m", "pip", "install", "-e", str(parent_dir)], check=False)
            return True
        print("⚠️  pysurrogate source not found in parent directory — API docs may fail")
        return False
...
def main():
    parser = argparse.ArgumentParser(description="pysurrogate documentation builder")
    ...
    if args.command == "clean":
        ...
        if args.fast:
            os.environ["PYSURROGATE_DOCS_FAST_MODE"] = "1"
